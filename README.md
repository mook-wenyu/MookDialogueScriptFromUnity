...
- 基本对话：`角色名: 对话内容`
- 旁白文本的格式：
  1. 冒号开头（推荐）：`:旁白内容` - 保留空格和格式
  2. 直接书写（不推荐，会跳过空格）
- 标签使用 `#` 开头，标签内容会一直读取直到遇到下一个标签符号或换行
  - 例如：`#标签内容` 会将"标签内容"作为一个完整的标签
  - 例如：`#标签1 #标签2` 会分别创建"标签1"和"标签2"两个标签
  - 标签内容可以包含冒号、引号等特殊字符，如：`#类型:重要 #ID:"NPC001"`
- 标签必须放在对话后的同一行
- 引号内的`#`符号不会被解析为标签，而是作为普通文本处理
